...
async function processMessageForDesk(buffer: string, seqno: number, desk: any, callback: (err?: any) => void): Promise<void> {
  try {
    ...
    let emailDate = parsed.date || new Date();
    let dateSource = 'parsed-date';
    
    if (!parsed.date && parsed.headers) {
      // Try to extract from Date header
      const dateHeader = parsed.headers.get('date');
      if (dateHeader) {
        try {
          const headerValue = typeof dateHeader === 'string' ? dateHeader : dateHeader.toString();
          emailDate = new Date(headerValue);
    ...
    // Simple and reliable CC extraction
    try {
      if (parsed.cc) {
        console.log(`📧 CC field found in email`);
        
        // Handle different CC formats from mailparser
        if (Array.isArray(parsed.cc)) {
          for (const cc of parsed.cc) {
            if (cc && cc.address) {
              ccRecipients.push(cc.address);
              console.log(`📧 Added CC: ${cc.address}`);
            }
          }
        } else if (parsed.cc.address) {
          ccRecipients.push(parsed.cc.address);
          console.log(`📧 Added single CC: ${parsed.cc.address}`);
        }
      }
      
      console.log(`📧 Total CC recipients extracted: ${ccRecipients.length}`, ccRecipients);
    } catch (error) {
      console.error(`📧 Error extracting CC recipients:`, error);
      ccRecipients = []; // Fallback to empty array
    }
    ...
    if (parsed.attachments && parsed.attachments.length > 0) {
      ...
      for (const attachment of parsed.attachments) {
        try {
          ...
          console.log(`📎 Successfully processed attachment: ${filename} (${attachmentData.size} bytes, ${attachmentData.contentType})`);
    ...
    if (relatedTicketId) {
      console.log(`📝 Adding reply to existing ticket #${relatedTicketId} for desk ${desk.name} with ${attachments.length} attachments`);
